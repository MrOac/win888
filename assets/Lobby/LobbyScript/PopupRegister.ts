...
namespace Lobby {
    ...
    export class PopupRegister extends Dialog {
            ...
            if (this.edbCodeDaiLi.string) {
                ...
                Http.get(Configs.App.API, repParams, (err, res) => {
                    ...
                    if (res.success == false) {
                        this.txtCodeCheck.string = "<color=#FF2C23>ខុសច្បាប់</color>";
                    }
                    else {
                        this.txtCodeCheck.string = "<color=#23FF23>មានសុពលភាព</color>";
                    }
                });
            }
        }
        public actRegister() {
            ...
            let checkUserNameRegister = username.toLowerCase().substring(0, 5);
            if(!Utils.isNumberIsChar(username)){
                App.instance.alertDialog.showMsg(App.instance.getTextLang('txt_check_change_pass'));
                return;
            }
            if (checkUserNameRegister === "guest"){
                App.instance.alertDialog.showMsg(App.instance.getTextLang("txt_register_check_guest_username_error"));
                return;
            }
        ...
        //    }
            ...
            let reqParams = { "c": 1, "un": username, "pw": password, "cp": captcha, "cid": this.captchaId, "ac": this.edbCodeDaiLi.string, "area": Configs.App.NAME_APP };
