...
export class TabVip {
    ...
    getVipPointInfo() {
        ...
        Http.get(Configs.App.API, { "c": 126, "nn": Configs.Login.Nickname }, (err, res) => {
            ...
            if (!res["success"]) {
                App.instance.alertDialog.showMsg("កំហុសក្នុងការតភ្ជាប់ សូមព្យាយាមម្តងទៀត.");
                return;
            }
            ...
            for (let i = 0; i < this.items.childrenCount; i++) {
                ...
                if (i < res["ratioList"].length) {
                    ...
                    item.getChildByName("btnReceive").on("click", () => {
                        App.instance.confirmDialog.show2("តើអ្នកប្រាកដថាចង់ទទួលបានរង្វាន់ vippoint មែនទេ?\nផ្គូផ្គងកម្រិត Vippoint បច្ចុប្បន្នរបស់អ្នក។ :\n" + Utils.formatNumber(Configs.Login.VipPoint * res["ratioList"][i]) + " កាក់", (isConfirm) => {
                            if (isConfirm) {
                                App.instance.showLoading(true);
...
export class TabBank {
    ...
    CreateBank() {
        let branch = this.editBranch.string.trim();
        ...
        data["br"] = encodeURIComponent(branch ? branch : "No Branch");;
        data["t"] = 0;
    ...
    UpdateBank() {
        ...
        if (this.editNameUpdate.string == "" || this.editNumberUpdate.string == "" || this.editBranchUpdate.string == "" || this.idBankChosing == -1) {
            App.instance.alertDialog.showMsg("សូមបំពេញព័ត៌មានទាំងអស់។.");
            return;
        }
        if (this.editName.string.indexOf(' ') == -1) {
            App.instance.alertDialog.showMsg("ម្ចាស់គណនីត្រូវមានកន្លែងទំនេរ 1.");
            return;
        }
        var self = this;
        var data = {};
...
export default class PopupProfile extends Dialog {
    ...
    @property(cc.Label)
    lblChangePass: cc.Label = null;
    @property(cc.Node)
    nodeNotification: cc.Node = null;
    @property(cc.Button)
    btnMessage:cc.Button = null;
    @property(cc.Button)
    btnChangePass: cc.Button = null;
    ...
    start() {
        ...
        BroadcastReceiver.register(BroadcastReceiver.PROTECT_ACCOUNT, () => {
            this.updateTextProtectAccount();
        ...
        MiniGameNetworkClient.getInstance().addListener((data) => {
            ...
            switch (inpacket.getCmdId()) {
                case cmd.Code.EXCHANGE_VIP_POINT: {
                    ...
                    switch (res.error) {
                        case 0:
                            App.instance.alertDialog.showMsg("សូមចុច \"ទទួលបាន OTP\" ឬទទួលបាន OTP តាមរយៈ OTP APP ហើយបញ្ចូលដើម្បីបន្ត.");
                            break;
                        case 1:
    ...
    protected onDisable(): void {
        this.nodeNotification.active = false;
    }
    updateTextProtectAccount() {
        if (Configs.Login.Nickname.substring(0, 5).toLowerCase() == "guest") {
            this.lblChangePass.string = "ការពារគណនីរបស់អ្នក";
            this.btnMessage.node.active = true;
            this.nodeNotification.active = false;
            this.btnChangePass.node.x = -370;
            this.btnMessage.node.x = -190;
        } else {
            this.lblChangePass.string = "ផ្លាស់ប្តូរពាក្យសម្ងាត់";
            this.btnMessage.node.active = false;
            this.nodeNotification.active = false;
            this.btnChangePass.node.x = -320;
        }
    }
    actMessage() {
        if (this.nodeNotification) {
            this.nodeNotification.active = !this.nodeNotification.active;
        }
    }
    actRule() {
