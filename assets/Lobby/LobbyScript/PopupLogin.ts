...
namespace Lobby {
    ...
    export class PopupRegister extends Dialog {
        ...
        public actLogin() {
            ...
            let checkUserNameLogin = username.toLowerCase().substring(0, 5);
            if(!Utils.isNumberIsChar(username)){
                App.instance.alertDialog.showMsg(App.instance.getTextLang('txt_check_change_pass'));
                return;
            }
            if(checkUserNameLogin === "guest"){
                App.instance.alertDialog.showMsg(App.instance.getTextLang("txt_login_check_guest_username_error"));
                return;
            }else{
