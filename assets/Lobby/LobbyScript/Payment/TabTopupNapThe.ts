...
export default class TabTopupNapThe extends TabTopupChuyenKhoan {
    ...
    show(data, providerName) {
        ...
        this.loadCardList();
        this.lbFree.string = "- ប្រភេទកាតនីមួយៗមានតម្លៃបញ្ចូលទឹកប្រាក់ខុសៗគ្នា.";
    }
    onBtnXacNhan() {
        ...
        Http.get(Configs.App.API, request, (err, res) => {
            ...
            //  cc.log(res);
            if (res.success == true) {
                App.instance.ShowAlertDialog("សំណើបញ្ចូលទឹកប្រាក់ត្រូវបានផ្ញើដោយជោគជ័យ!\nសូមរង់ចាំពីរបីនាទី!");
            }
            else {
                App.instance.ShowAlertDialog("ព័ត៌មានកាតមិនត្រឹមត្រូវ។\nសូមពិនិត្យម្តងទៀត!");
            }
        });
    }
    loadCardList() {
        ...
        this.lobbyChoseBank.init(this.tabWell, this.data.banks, (dataBankChosing) => {
            ...
            for (let i = 0; i < this.listRateCard.length; i++) {
                if (this.dataBankChosing['key'] == this.listRateCard[i]['type']) {
                    this.lbFree.string = "- បញ្ចូលថ្លៃសេវា: " + this.listRateCard[i]['fee'];
                }
            }
