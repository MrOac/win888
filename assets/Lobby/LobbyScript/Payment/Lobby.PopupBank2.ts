...
@ccclass
export default class TabPopupBanks2 extends Dialog {
    @property(cc.Label)
    ...
    start() {
        for (let i = 0; i < this.contentPackage.length; i++) {
            ...
            let heightContent = this.contentPackage[i].height;
            this.totalHeightContent += heightContent;
        }
    ...
    onScrollEvent() {
        ...
        let lowerLimit = this.totalHeightContent - this.liskPackage.height;
        if (upperLimit < 0) {
