...
const NAME_TABS = { "paywell": 0, "royalpay": 1, "princepay": 2, "manualbank": 3, "clickpay": 4, "payasec": 5, "chuyenkhoan": 6, "napthe": 7, "bank": 8, "gift": 9};
            ...
            if (Configs.Login.ListPayment[i].providerConfig.status == 1) {
                if (NAME_TABS[Configs.Login.ListPayment[i].providerName] != null) {
                    if (this.btnTabs[NAME_TABS[Configs.Login.ListPayment[i].providerName]]) {
                        ...
                        if (isFind == false) {
                            this.onBtnChoseTab(null, Configs.Login.ListPayment[5].providerName);
                            isFind = true;
                        }
