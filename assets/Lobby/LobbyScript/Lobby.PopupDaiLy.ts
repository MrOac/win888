...
export default class PopupDaiLy extends Dialog {
    ...
    onClickItemDetail(even) {
        ...
        Http.get(Configs.App.API, { "c": 20331, "code": idAgency, "nn": Configs.Login.Nickname, "at": Configs.Login.AccessToken }, (err, res) => {
            App.instance.showLoading(false);
            //  cc.log(res);
            if (err != null) return;
            if (res["success"] && res['data'] != null) {
    ...
    edbChanged() {
        this.lbChipReceive.string = this.edbMoney.string + " Coin";
    ...
    onClickConfirm() {
        ...
        let transactionID = this.edbTransID.string.trim();
        if (this.tab == TAB.CASHIN) {
            if (agencyName == "" || nickname == "" || amount == 0 || transactionID == "" || this.currentDataDaily == null || this.currentDataUser == null) {
    ...
    onChangeTab(even, data) {
        ...
        switch (parseInt(data)) {
            case TAB.CASHIN:
                ...
                this.edbTransID.node.active = true;
