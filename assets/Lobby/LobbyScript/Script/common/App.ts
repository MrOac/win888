...
export default class App extends cc.Component {
  ...
  //  public popupGameSBO: PopupGameSBO = null;
    ...
    static DOMAIN = "bitjackpot.io";
    static API_CMD: string = "https://ga.bitjackpot.io/3rd/cmd";
    static API_IBC: string = "https://ga.bitjackpot.io/3rd/ibc";
    static API_SBO: string = "https://ga.bitjackpot.io/3rd/sbo"
    static API_AG: string = "https://ga.bitjackpot.io/3rd/ag";
    static API_EBET: string = "https://ga.bitjackpot.io/3rd/ebet";
    static API_WM: string = "https://ga.bitjackpot.io/3rd/wm";

    //http://localhost:8081/api?c=2021&nn=tuanbigbird&at=1628224022&ip=127.0.0.1&mn=100
    updateConfigGame(domain = "bitjackpot.io") {
                    ...
                    Http.get(App.API_IBC, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                        ...
                        if (res["code"] == 0) {
                            if (Configs.App.IS_PRO == true && Configs.Login.UserType != "2") {
                                var url = "https://bitjackpot.io/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "https://bitjackpot.io/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);

                            }
                            else {
                                var url = "http://bitjackpot.io/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                if (cc.sys.isMobile == true) {
                                    url = "http://bitjackpot.io/deposit_processlogin.aspx?lang=vn&token=" + res["data"]["data"];
                                }
                                cc.sys.openURL(url);
                            }
                        }
                ...
                if (res["data"] == null) {
                    ...
                    if (balance < 10000 && isPlayNow == false) {
                        ...
                        Http.get(App.API_EBET, { t: "Login", nn: Configs.Login.Nickname, at: Configs.Login.AccessToken }, (err, res) => {
                            App.instance.showLoading(false);
                            if (res["res"] == 0) {
                                let url = "https://bitjackpot.io/h5/72895c?username=%s&accessToken=%s";
