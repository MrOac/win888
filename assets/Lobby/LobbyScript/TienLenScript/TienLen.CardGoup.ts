...
class CardGroup {
	...
	getCardType() {
		...
		switch (mapSize) {
			...
			// break;
			case 2:
				if (newMap.get(keys[0]).length == 2){
					return TypeOfTurn.HAI_DOI_THONG;
				} 
				else return TypeOfTurn.HAI_TU_QUY;
			// break;
			case 3:
...
// dữ liệu khi chọn một lá  data: {card: 10, face: 3} , ListSelected [Card] Card là một Object
	getSuggestionNoCards(listSelected, data, isCheck = false) {
		...
		if (!isCheck) {
			...
			listSuggestion = this.get_HAI_DOI_THONG([], listSelected);
			if (listSuggestion.length > 0) {
				return listSuggestion;
			}
	...
	getSuggestionCards(submitCards, data, callback) {
		...
		switch (submitCardType) {
			...
			case TypeOfTurn.HAI_TU_QUY:
				...
				break;
			case TypeOfTurn.HAI_DOI_THONG:
				listSuggestion = this.get_HAI_DOI_THONG(submitCards);
				break;
			case TypeOfTurn.BA_DOI_THONG:
				listSuggestion = this.get_BON_DOI_THONG(submitCards);
				if (listSuggestion.length > 0)
	...
	get_HAI_DOI_THONG(submitCards, listSelected = null) {
		if (` ${submitCards}`.length > 0) {
			let listCard = new Array();
			...
			for (let i = 0; i < listTmp2.length; i++) {
				for (let j = 0; j < listTmp2[i].length - 1; j++) {
					let tmp = new Array();
					for (let l = j; l < j + 2; l++) {
						for (let k = 0; k < 2; k++)
