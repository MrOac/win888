...
export default class InGame extends cc.Component {
    ...
    firstTurn(data) {
        ...
        this.fxWhoPlayFirst.getChildByName("txt").getComponent(cc.Label).string = this.cachePlayersInfo[data.chair] + " ទៅមុន !";
        this.scheduleOnce(() => {
    ...
    passTurn(turn) {
         //Utils.Log("Pass turn");
        this.players[this.convertChair(turn.chair)].setNotify("ឆ្លងកាត់");
        this.setActiveButtons(["bt_submit_turn", "bt_pass_turn"], [false, false]);
        this.players[0].setTimeRemain(0);
    }
    ...
    checkSuggestion(data) {
        // data chỉ nhập một giá trị tại một thời điểm vd: {card: 8, face: 2}
        ...
        var suggestionCards;
        //this.checkTurn luot minh đánh(false), hay lượt mình đỡ(true)
        if (this.checkTurn){
    ...
    wait4doithong(res) {
        ...
        this.fxWhoPlayFirst.active = true;
        this.fxWhoPlayFirst.getChildByName("txt").getComponent(cc.Label).string = "រង់ចាំបួនគូនៃស្រល់ !";
        setTimeout(() => {
            this.fxWhoPlayFirst.active = false;
        }, 2000);
    }
