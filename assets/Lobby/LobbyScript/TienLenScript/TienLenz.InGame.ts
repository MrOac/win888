...
export default class InGame extends cc.Component {
    ...
    autoStart(autoInfo) {
        if (autoInfo.isAutoStart) {
            this.setTimeCountDown("ហ្គេមចាប់ផ្តើមនៅពេលក្រោយ: ", autoInfo.autoStartTime);
            this.closePopupResult();
        }
    }
    ...
    firstTurn(data) {
        ...
        this.timeoutDelayChiaBai = setTimeout(() => {
            ...
            this.fxWhoPlayFirst.children[0].getComponent(cc.Label).string = this.cachePlayersInfo[data.chair] + " លេងមុន។ !";
        }, 1000);
    ...
    submitTurn(turn) {
        ...
        if (isExist2) {
            this.fxWhoPlayFirst.active = true;
            this.fxWhoPlayFirst.children[0].getComponent(cc.Label).string = "ពីរ !";
            setTimeout(() => {
                this.fxWhoPlayFirst.active = false;
            }, 2000);
        }
    ...
    passTurn(turn) {
        this.players[this.convertChair(turn.chair)].setStatus("ឆ្លងកាត់");
        this.setActiveButtons(["bt_submit_turn", "bt_pass_turn"], [false, false]);
        this.players[0].setTimeRemain(0);
    }
    ...
    endGame(data) {
        ...
        if (data.countDown == 0) {
            ...
            this.setTimeCountDown("ហ្គេមបញ្ចប់នៅពេលក្រោយ: ", 10);
            setTimeout(() => {
                this.cleanCardsOnHand();
                this.cleanCardLine();
                this.cleanCardsOnBoard();
            ...
            setTimeout(() => {
                this.setTimeCountDown("ហ្គេមបញ្ចប់នៅពេលក្រោយ: ", data.countDown - 6);
            }, 4000);
    ...
    notifyUserRegOutRoom(res) {
        ...
        if (chair == 0) {
            if (isOutRoom) {
                this.players[chair].setNotify("ហៀបនឹងចាកចេញ !");
            } else {
                this.players[chair].setNotify("លេងទៅកំពង់ផែ !");
            }
        }
    }
    ...
    wait4doithong(res) {
        ...
        this.fxWhoPlayFirst.active = true;
        this.fxWhoPlayFirst.children[0].getComponent(cc.Label).string = "រង់ចាំបួនគូនៃស្រល់ !";
        setTimeout(() => {
            this.fxWhoPlayFirst.active = false;
        }, 2000);
    }
