...
export default class PopupDetailHistory extends Dialog {
    ...
    show() {
        ...
        this.lblSession.string = "ជុំ: #" + this.session;
    ...
    private loadData() {
        ...
        this.sprDice3.node.active = false;
        // this.sprResult.node.active = false;
        this.lblSession.string = "ជុំn: #" + this.session;
        this.lblResult.string = "";
        this.totalBetTai = 0;
        this.totalBetXiu = 0;
        ...
        Http.get(Configs.App.API, { "c": 102, "rid": this.session, "mt": Configs.App.MONEY_TYPE }, (err, res) => {
            ...
            if (res.success && res["resultTX"] !== null) {
                ...
                this.lblSession.string = "ជុំ: #" + res["resultTX"]["referenceId"];
                this.lblResult.string = res["resultTX"]["result"] == 1
                    ? " - ធំ " + (res["resultTX"]["dice1"] + res["resultTX"]["dice2"] + res["resultTX"]["dice3"]) + "(" + res["resultTX"]["dice1"] + "-" + res["resultTX"]["dice2"] + "-" + res["resultTX"]["dice3"] + ")"
                    : " - តូច " + (res["resultTX"]["dice1"] + res["resultTX"]["dice2"] + res["resultTX"]["dice3"]) + "(" + res["resultTX"]["dice1"] + "-" + res["resultTX"]["dice2"] + "-" + res["resultTX"]["dice3"] + ")";
