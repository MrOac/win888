...
export default class LotoController extends cc.Component {
    ...
    //    cc.log(this.sessionDate);
     ...
     //   cc.log("CoinFish : ", Configs.Login.CoinFish);
        ...
        ShootFishNetworkClient.getInstance().checkConnect((isLogined) => {
            if (!isLogined) {
                App.instance.alertDialog.showMsgWithOnDismissed("ការចូលបានបរាជ័យ សូមព្យាយាមម្តងទៀត.", () => {
                    this.actBack();
                });
                return;
            }
        //    Play.SERVER_CONFIG = Configs.Login.FishConfigs;
            ...
            if (Configs.Login.CoinFish <= 0) {
                App.instance.confirmDialog.show3("លុយនៅក្នុងឆ្នោតរបស់អ្នកអស់ហើយ តើអ្នកចង់ផ្ទេរវាទេ?", "មាន", (isConfirm) => {
                    if (isConfirm) {
        ...
        ShootFishNetworkClient.getInstance().addOnClose(() => {
            App.instance.showErrLoading("បាត់ការតភ្ជាប់ ព្យាយាមភ្ជាប់ម្តងទៀត...");
        }, this);
    ...
    actionSubmitBet() {
        ...
        if ((hourOfDay == 18 && minOfDay > 5) || hourOfDay >= 19) {
            let msg_1 = "អស់ពេលភ្នាល់។\nម៉ោងភ្នាល់ចាប់ពីម៉ោង 0h ដល់ 18h05.";
       ...
       // kiem tra so luong so can danh cua mode do
        if (this.numRequired == 1) {
            if (this.currentNumPicked.length < 1) {
                let msg_3 = "អ្នកត្រូវជ្រើសរើសយ៉ាងហោចណាស់ 1 លេខ!";
                this.showPopupNotify(msg_3);
                return;
            }
        } else {
            if (this.currentNumPicked.length !== this.numRequired) {
                let msg_3 = "អ្នកត្រូវជ្រើសរើស " + this.numRequired + " ចំនួន !";
                this.showPopupNotify(msg_3);
                return;
            }
        }
        if (this.currentBetValue == 0) {
            this.showPopupNotify("សូមជ្រើសរើសវិទ្យុភាគខាងជើងម្តងទៀត");
            return;
        }
    ...
    //    cc.log("Loto actionSubmitBet totalBet : ", totalBet);
      ...
      //      cc.log("Loto actionSubmitBet Du tien");
        ...
        //    cc.log("Loto actionSubmitBet Khong du tien");
            this.showPopupNotify("អ្នកមិនមានលុយគ្រប់គ្រាន់ទេ!");
        }
    }
     ...
     //   cc.log("LOTO onTextChangeBet event: ", event);
        if (event.length > 0) {
            if (/^[0-9]*$/.test(event) == false) {
                App.instance.alertDialog.showMsg("ការភ្នាល់ត្រូវតែជាលេខវិជ្ជមាន");
                this.edtBet.string = "1";
                event = "1";
            }
        ...
        ShootFishNetworkClient.getInstance().request("LOTO1", {
         ...
         //   cc.log("LOTO1 : ", res);
            if (res["code"] != 200) {
                ...
                App.instance.alertDialog.showMsg("កំហុស " + res["code"] + ", " + res["msg"]);
                return;
            }
         ...
         //   cc.log("LOTO1 numRequest : ", this.numRequest);
            if (this.numRequest == this.numRequestCompleted) {
                this.showPopupNotify("កំណត់ជោគជ័យ!");
...
actLogin(): void {
           let username = Configs.Login.Username;
            ...
            App.instance.showLoading(true);
            Http.get(Configs.App.API, { c: 3, un: username, pw: password }, (err, res) => {
                App.instance.showLoading(false);
                if (err != null) {
                    App.instance.alertDialog.showMsg("ការចូលបានបរាជ័យ សូមពិនិត្យមើលការតភ្ជាប់ម្តងទៀត");
                    return;
                }
                // console.log(res);
                switch (parseInt(res["errorCode"])) {
                   ...
                   //     this.getMailNotRead();
                        ...
                        break;
                    case 1007:
                        App.instance.alertDialog.showMsg("ព័ត៌មានចូលមិនត្រឹមត្រូវ។");
                        break;
                    case 2001:
                        this.popupUpdateNickname.show2(username, password);
                        break;
                    default:
                        App.instance.alertDialog.showMsg("ការចូលបានបរាជ័យ សូមព្យាយាមម្តងទៀតនៅពេលក្រោយ។");
                        break;
                }
            });
        }
