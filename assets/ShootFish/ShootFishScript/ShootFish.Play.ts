...
export default class Play extends cc.Component {
            ...
            switch (route) {
                ...
                case "OnLeavePlayer": {
                    ...
                    if (username == Configs.Login.UsernameFish) {
                        if (data["reason"] == 1) {
                            App.instance.alertDialog.showMsg("មិនចូលរួម.");
                        }
                        this.back();
                    }
                ...
                case "OnJackpot": {
                    ...
                    let msg = "សូមអបអរសាទរ \"" + nickname + "\" បរបាញ់ " + Utils.formatNumber(value) + " កាក់ចូល " + roomName + "."
                    break;
                }
                default:
                    // //  cc.log("====route: " + route);
                ...
                ShootFishNetworkClient.getInstance().ping(() => {
                    ...
                    ShootFishNetworkClient.getInstance().request("play", {
                        ...
                        if (!res["ok"]) {
                            switch (res["err"]) {
                                case 4:
                                    App.instance.confirmDialog.show2("ទឹកប្រាក់មិនគ្រប់គ្រាន់.", (isConfirm) => {
                                        if (isConfirm) {
                                ...
                                case 1:
                                    ...
                                    App.instance.alertDialog.showMsg("កំហុស " + res["err"] + ", សូម​ព្យាយាម​ម្តង​ទៀត។.");
                                    break;
                                default:
                                    App.instance.alertDialog.showMsg("កំហុស " + res["err"] + ",មិនស្គាល់.");
                                    break;
                            }
                            this.show(false);
    ...
    public actBack() {
        App.instance.confirmDialog.show2("តើអ្នកចង់ចាកចេញពីតុទេ?", (isConfirm) => {
            if (isConfirm) {
                this.back();
            }
        });
    ...
    public show(isShow: boolean, roomId: number = 0) {
        if (isShow) {
            if (Play.SERVER_CONFIG == null) {
                this.lobby.getComponent(Lobby).show(true);
                App.instance.alertDialog.showMsg("អ្នកមិនបានចូលទេ។.");
                return;
            }
